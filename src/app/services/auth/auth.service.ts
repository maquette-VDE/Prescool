...
export class AuthService {
  ...
  login(username: string, password: string) {
    ...
    return this.http
      .post<any>(this.apiConfig.buildUrl('auth/token'), body.toString(), {
        headers,
      })
      .pipe(
        tap((response) => {
        ...
        switchMap(() =>
          this.http.get<any>(this.apiConfig.buildUrl('users/me')),
        ),
        tap((user) => {
  ...
  refreshToken(): Observable<{ accessToken: string }> {
    ...
    return this.http
      .post<{
        accessToken: string;
      }>(this.apiConfig.buildUrl('auth/refresh'), { refreshToken }, { observe: 'response' })
      .pipe(
        map((response) => {
  ...
  logout(): void {
    this.http.post(this.apiConfig.buildUrl('users/me/logout'), {}).subscribe({
